...
import com.ithaibo.sample.fragment.TouchEventFragment;
...
public class FragmentContainerActivity extends BaseActivity implements View.OnClickListener, ListDialogItemPresenter<Fragment> {
	...
	protected void initData() {
		if (menuDialogDataList == null) {
			...
			menuDialogDataList.add(new ListDialogItemDataBean<>(new CircleViewFragment(), "CircleView", this));
			menuDialogDataList.add(new ListDialogItemDataBean<>(new CustomeHorizontalScrollViewFragment(), "CustomeHorizontalScrollView", this));
			menuDialogDataList.add(new ListDialogItemDataBean<>(new PathFragment(), "Path", this));
			menuDialogDataList.add(new ListDialogItemDataBean<>(new MatrixFragment(), "matrix", this));
			menuDialogDataList.add(new ListDialogItemDataBean<>(new TouchEventFragment(), "touch event", this));
		}
	}
	...
	protected void onStart() {
		super.onStart();
		if (menuDialogDataList.size() > 0)
			doReplaceFragment(menuDialogDataList.get(menuDialogDataList.size() - 1).getItemData());
	}
